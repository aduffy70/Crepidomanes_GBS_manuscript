...
Analysis of K-means and Evanno's delta-K for the C. intricatum + mixed/unknown data subsets suggested that the samples cluster optimally into 2--6 groups (Figure 9; Figure 11), but for the _C. intricatum_ samples the optimal number of clusters appears to be just one (Figure 10; Figure 11). Structure results using K=2 divide the ancestry for every sample between two clusters (Figure 8a), and at higher values, samples with the lowest number of loci (S45, S34, S04) begin to group separately (Figure 8b).
